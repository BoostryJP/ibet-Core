...
func (r *reporter) send() error {
	...
	r.reg.Each(func(name string, i interface{}) {
		...
		case metrics.Counter:
			v := metric.Count()
			l := r.cache[name]
			if p, err := client.NewPoint(fmt.Sprintf("%s%s.count", namespace, name), r.tags, map[string]interface{}{
				"value": v - l,
			}, now); err == nil {
				pts = append(pts, p)
			}
		...
		case metrics.Gauge:
			ms := metric.Snapshot()
			if p, err := client.NewPoint(fmt.Sprintf("%s%s.gauge", namespace, name), r.tags, map[string]interface{}{
				"value": ms.Value(),
			}, now); err == nil {
				pts = append(pts, p)
			}
		...
		case metrics.Histogram:
			...
			if ms.Count() > 0 {
				...
				if p, err := client.NewPoint(fmt.Sprintf("%s%s.histogram", namespace, name), r.tags, map[string]interface{}{
					"count":    ms.Count(),
					"max":      ms.Max(),
					"mean":     ms.Mean(),
					"min":      ms.Min(),
		...
		case metrics.Meter:
			ms := metric.Snapshot()
			if p, err := client.NewPoint(fmt.Sprintf("%s%s.meter", namespace, name), r.tags, map[string]interface{}{
				"count": ms.Count(),
				"m1":    ms.Rate1(),
				"m5":    ms.Rate5(),
				"m15":   ms.Rate15(),
		...
		case metrics.Timer:
			ms := metric.Snapshot()
			ps := ms.Percentiles([]float64{0.5, 0.75, 0.95, 0.99, 0.999, 0.9999})
			if p, err := client.NewPoint(fmt.Sprintf("%s%s.timer", namespace, name), r.tags, map[string]interface{}{
				"count":    ms.Count(),
				"max":      ms.Max(),
				"mean":     ms.Mean(),
				"min":      ms.Min(),
		...
		case metrics.ResettingTimer:
			...
			if len(t.Values()) > 0 {
				...
				val := t.Values()
				if p, err := client.NewPoint(fmt.Sprintf("%s%s.span", namespace, name), r.tags, map[string]interface{}{
					"count": len(val),
					"max":   val[len(val)-1],
					"mean":  t.Mean(),
					"min":   val[0],
